...
- What are telomeres?: https://en.wikipedia.org/wiki/Telomere (1 pt)
...
How many variables in each category by survey year? (1 pt)
...
M # the number of X variables in our XWAS
```
...
We will now view the distribution of the variables (2 pt)
```{r}
...
- How large are the association sizes for telomere length? (1 pt)
- What is the interpretation of each coefficient in telomere length? Which variable dominates, if any? (1 pt)
- which ones can you say are "statistically" not different than zero? (1 pt)
- interpret the coefficients. (2 pt)

```{r}
summary(mod)
```

One can also scale the TELOMEAN variable (as we did in the paper) by the standard deviation - how does this change the interpretation? (1 pt)
...
First we define some 'helper' functions that do the individual association tests (using linear regression) in step 3a (1 pt):
...
### Required Questions:
1. What does telomere length measure? (1 pt)

2. What is your interpretation of the association sizes and pvalues for your XWAS? (1 pt)
- Hint: "For a 1SD change in the natural log of the biomarker of exposure = ???? change in telomere length..."

3. Recall the chunk of code under heading 4, "Conducting initial survey-weighted analyses of the Y and X". What are the associations between sociodemographic factors for a basepair change in mean telomere length? (1 pt)
...
What is the max association size between any variant implicated in the GWASs and telomere length? (1 pt)
